...
const Section = ({ title, to, children }: SectionI) => {
    ...
    return (
        <section className="bg-gray p-3 rounded-md flex flex-col gap-2">
            <h2 className="font-monsterrat text-xl font-bold">{title}</h2>
            <article>
                {children}
...
const Home = () => {
    return (
        <div className="flex flex-col gap-3">
            <h1 className="font-monsterrat text-3xl">Institchu Code Test</h1>
            <Section title="Task 1" to={AppRoute.TASK_1}>
                <div className="text-lg">Photo album</div>
                <p>Find out how https://jsonplaceholder.typicode.com/ REST API works and make a web page to display all photos in album id:1.</p>
            </Section>
            <Section title="Task 2 & 3" to={AppRoute.TASK_2}>
                <div className="text-lg">New User Form</div>
                <ol>
